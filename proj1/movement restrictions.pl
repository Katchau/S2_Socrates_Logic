...
jump(Board, X, Y):- X == 0; Y == 0; check_ortho_adjacency(Board, X, Y).
jump(Board, X0, Y0, X, Y):- X \== X0, Y0 == Y, X_F is X + (X-X0), jump(Board, X_F, Y).
jump(Board, X0, Y0, X, Y):- Y \== Y0, X0 == X, Y_F is Y + (Y-Y0), jump(Board, X, Y_F).
jump(Board, Player, X0, Y0, X, Y):- select_piece(Board, X, Y, Peca), Peca \== Player, jump(Board, X0, Y0, X, Y).
